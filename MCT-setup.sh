#!/bin/bash
# MCT Masternode Setup Script V1.3 for Ubuntu 16.04 LTS
...
#
# Usage:
# bash MCT-setup.sh [Masternode_Private_Key]
#
# Example 1: Existing genkey created earlier is supplied
# bash MCT-setup.sh 27dSmwq9CabKjo2L3UD1HvgBP3ygbn8HdNmFiGFoVbN1STcsypy
#
...
function stop_daemon {
    if pgrep -x 'mctd' > /dev/null; then
        echo -e "${YELLOW}Attempting to stop mctd${NC}"
        mct-cli stop
        delay 30
        if pgrep -x 'mctd' > /dev/null; then
            echo -e "${RED}mctd daemon is still running!${NC} \a"
            echo -e "${YELLOW}Attempting to kill...${NC}"
            pkill mctd
            delay 30
            if pgrep -x 'mctd' > /dev/null; then
                echo -e "${RED}Can't stop mctd! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
clear
echo -e "${YELLOW}(c) 2018 by Rush Hour, MCT Masternode Setup Script V1.3 for Ubuntu 16.04 LTS${NC}"
...
cp ~/MCTmasternodesetup/nodemon.sh /usr/local/bin
sudo chmod 711 /usr/local/bin/nodemon.sh

#Create datadir
if [ ! -f ~/.mctcore/mct.conf ]; then 
	sudo mkdir ~/.mctcore
fi

echo -e "${YELLOW}Creating mct.conf...${NC}"

# If genkey was not supplied in command line, we will generate private key on the fly
if [ -z $genkey ]; then
    cat <<EOF > ~/.mctcore/mct.conf
rpcuser=$rpcuser
rpcpassword=$rpcpassword
EOF

    sudo chmod 755 -R ~/.mctcore/mct.conf
   ...
   echo -ne '[##                 ] (15%)\r'
    ...
    genkey=$(mct-cli masternode genkey)
    if [ -z "$genkey" ]; then
    ...
    #Stopping daemon to create mct.conf
    stop_daemon
    echo -ne '[##                 ] (15%)\r'
    sleep 6
    echo -ne '[######             ] (30%)\r'
...
sudo git clone https://github.com/swatchie-1/sentinel.git /root/sentinel-mct
cd /root/sentinel-mct
virtualenv venv
. venv/bin/activate
pip install -r requirements.txt
export EDITOR=nano
(crontab -l -u root 2>/dev/null; echo '* * * * * cd /root/sentinel-mct && ./venv/bin/python bin/sentinel.py >/dev/null 2>&1') | sudo crontab -u root -

#Setting auto star cron job for mctd
cronjob="@reboot sleep 30 && mctd -daemon"
crontab -l > tempcron
...
for your Hot Wallet (the wallet with your 10,000 or 50,0000 MCT collateral funds):
======================================================================== \a"
...
To introduce your new masternode to the MCT network, you need to
...
Currently your masternode is syncing with the MCT network...
...
========================================================================
To view masternode configuration produced by this script in mct.conf:
${YELLOW}cat ~/.mctcore/mct.conf${NC}
Here is your mct.conf generated by this script:
-------------------------------------------------${YELLOW}"
cat ~/.mctcore/mct.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit mct.conf, first stop the mctd daemon,
then edit the mct.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the mctd daemon back up:
to stop:   ${YELLOW}mct-cli stop${NC}
to edit:   ${YELLOW}nano ~/.mctcore/mct.conf${NC}
to start:  ${YELLOW}mctd${NC}
========================================================================
To view mctd debug log showing all MN network activity in realtime:
${YELLOW}tail -f ~/.mctcore/debug.log${NC}
========================================================================
...
========================================================================
Enjoy your MCT Masternode and thanks for using this setup script!
If you found this script and masternode setup guide helpful...,
...please donate to the devfound: MCT  **BMEfHRvfcwoPgpBgpEVERsQpipNjtFVgVL**,
